...
public class ColorPickerDialog extends AlertDialog {
    ...
    protected void onCreate(Bundle savedInstanceState) {
		...
		color = new AppCompatEditText(context);
		color.setText("#ffffffff");
		int padd = (int)(height*0.05f);
		TextView tv = new TextView(context);
		tv.setText(title+ "(修改自第三方)");
		tv.setTextColor(Color.BLACK);
		AppCompatButton copy = new AppCompatButton(context);
		copy.setText("复制");
		copy.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				ClipboardManager clip = (ClipboardManager) context.getSystemService(Context.CLIPBOARD_SERVICE);
				clip.setText(color.getText().toString());
				CharSequence a = clip.getText();
    ...
    private class ColorPickerView extends View {
		...
		public ColorPickerView(Context context, int height, int width) {
            ...
            mPaint.setStrokeWidth(mHeight*0.15f);
            ...
            centerRadius = (r - mPaint.getStrokeWidth() / 2 ) * 0.4f;
            ...
            rectTop = r + mPaint.getStrokeWidth() * 0.5f + 
            		mLinePaint.getStrokeMiter() * 0.5f + 15+30;
            ...
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                	if(downInCircle && inCircle) {//down按在渐变色环内, 且move也在渐变色环内
						...
						int a  = interpCircleColor(mCircleColors, unit);
	               		mCenterPaint.setColor(a);
						color.setText("#"+Integer.toHexString(a));
	               		if(debug) Log.v(TAG, "色环内, 坐标: " + x + "," + y);
                	}else if(downInRect && inRect) {//down在渐变方块内, 且move也在渐变方块内
						int a = interpRectColor(mRectColors, x);
                		mCenterPaint.setColor(a);
						color.setText("#"+Integer.toHexString(a));
                	}
